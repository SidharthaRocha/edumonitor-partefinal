...
const CadastroAluno = () => {
  ...
  const handleCadastro = async () => {
    ...
    try {
      const response = await fetch('https://c85b-177-10-253-248.ngrok-free.app/backend/cadastro_aluno.php', { 
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('userName', nome); // Salva o nome no localStorage
        navigate('/aluno'); // Redireciona após o cadastro
      } else {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-purple-900 font-poppins">
      ...
      >
        ...
        <h2 className="text-2xl font-semibold mb-4 text-purple-900">Cadastro de Aluno</h2>
        ...
        <Link to="/aluno" className="text-purple-600 mb-2 block hover:underline">Possui um cadastro? Faça Login</Link>
